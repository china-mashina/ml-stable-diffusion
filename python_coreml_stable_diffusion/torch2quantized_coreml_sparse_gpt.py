...
#
"""Quantize Stable Diffusion XL UNet using SparseGPT and convert to Core ML."""
...
@FirstLayerInputCacher.register("multi_input")
class MultiInputCacher(FirstLayerInputCacher):
    """Cache multi-tensor inputs for models such as UNet."""
...
def sparsegpt_config(calibration_nsamples):
    """Return LayerwiseCompressorConfig for SparseGPT weight compression."""

    logger.info("Creating SparseGPT config")
    config = LayerwiseCompressorConfig.from_dict(
        {
            "global_config": {"algorithm": "sparse_gpt", "weight_dtype": "uint8"},
            "input_cacher": "multi_input",
...
def quantize(model, config, calibration_data):
    """Compress weights using SparseGPT with calibration data."""
    ...
    compressed_model = compressor.compress(dataloader(), device=str(device))
    _log_device("Compressed model", compressed_model)
...
def convert_quantized_unet(pipe, args):
    ...
    # Quantize UNet weights with SparseGPT
    config = sparsegpt_config(len(dataloader))
    logger.info("Quantizing UNet model with SparseGPT")
